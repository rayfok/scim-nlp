...
from scienceparseplus.constants import IMAGE_FEATURE_SIZE
from scienceparseplus.modeling.image_feature_encoder import create_image_feature_encoder
...
parser.add_argument(
    "--use_image_features",
    action="store_true",
    help="If set, it will use image_features when modeling",
)
...
class DocBankSimpleTokenier:
    ...
    def encode_plus(self, features):
        ...
        if self.use_image_features:
            ...
            #image_shape = Counter([f["image"].shape[:2] for f in features]).most_common()[0][0]
            #encoded_inputs["images"] = torch.stack([torch.from_numpy(cv2.resize(f["image"], image_shape)) for f in features], dim=0)
            
            encoded_inputs["images"] = [f["image"] for f in features]
...
def load_model_and_tokenizer(args, num_labels, config):
    ...
    if "layoutlm" in args.model_name.lower():
        ...
        tokenizer = DocBankSimpleTokenier(tokenizer, use_layout=True, use_image_features=args.use_image_features)

    else:
        tokenizer = AutoTokenizer.from_pretrained(args.model_name)
        tokenizer = DocBankSimpleTokenier(
            tokenizer, use_layout=args.add_2d_pos_for_bert
        )
    ...
    if args.use_image_features:
        ...
        model.image_encoder = create_image_feature_encoder(
                config["image_encoder_model_path"]
            )
...
if __name__ == "__main__":
    ...
    if not args.use_image_features:
        ...
        from scienceparseplus.modeling.layoutlm import LayoutLMForTokenClassificationWithImageFeature as LayoutLMForTokenClassification
    ...
    if not args.use_image_features:
        ...
        datasets = {
            subset: DocBankImageFeatureDataset(
                base_path=config["Data"]["doc_bank_base_path"],
                subset=subset,
                image_directory=config["Data"]["doc_bank_image_path"],
                select_n=select_n,
                select_ratio=select_ratio,
                filename=f"{subset}-token.json",
                add_class_weight=args.add_class_weight,
    ...
    if args.use_image_features:
        model_folder = model_folder + "-use_image_features"
