...
class LayoutLMEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.block_embeddings = nn.Embedding(
            MAX_BLOCK_EMBEDDING_NUMBER, config.hidden_size
        )
    ...
    def forward(
        ...
        if block_ids is None:
            block_ids = torch.zeros(input_shape, dtype=torch.long, device=device)
