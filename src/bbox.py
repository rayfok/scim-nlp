...
class SppBBox:
    def __init__(
        self, left: float, top: float, right: float, bottom: float, page_id: int
    ):
        self.left = left
        self.top = top
        self.right = right
    ...
    @staticmethod
    def normalize_spp_bbox_json(
        spp_bbox_json: Dict, page_id: int, spp_page_height: float, spp_page_width: float
    ) -> BoundingBox:
        """Simply converts a scienceparseplus JSON box into the boxes we need for scholarphi"""
        left, top, right, bottom = spp_bbox_json
        spp_bbox = SppBBox(
            left=left, top=top, right=right, bottom=bottom, page_id=page_id
        )
        return BoundingBox(
            left=spp_bbox.left / spp_page_width,
...
def _union_bboxes(bboxes: List[BoundingBox]) -> BoundingBox:
    ...
    return BoundingBox(
        page=bboxes[0].page, left=x1, top=y1, width=x2 - x1, height=y2 - y1
    )
...
class TokenWithBBox:
    ...
    def to_json(self) -> Dict:
        return {"text": self.text, "bbox": _bbox_to_json(bbox=self.bbox)}
...
class Block:
    ...
    def __init__(self, tokens: List[TokenWithBBox], bbox: BoundingBox, type: str):
        ...
        # form rows
        self.rows = [
            RowWithBBox(tokens=[self.tokens[id] for id in row_token_ids])
            for row_token_ids in self.token_row_clusters
        ]

        # map sentences into char index space
        self.sent_char_spans: List[
            Tuple
        ] = self._compute_sent_char_spans()  # O(num_sents)
    ...
    def _compute_token_char_spans(
        self, tokens: List[Union[TokenWithBBox, SymbolWithBBoxes]]
    ) -> List[Tuple]:
    ...
    def _find_sent_idx_for_this_token_char_span(
        self, token_start: int, token_end: int
    ) -> int:
    ...
    def _cluster_tokens_by_sent(self) -> List[List[int]]:
        ...
        for token_idx, (token_start, token_end) in enumerate(self.token_char_spans):
            sent_idx = self._find_sent_idx_for_this_token_char_span(
    ...
    def _compute_sent_bboxes(self, token_sent_cluster: List[int]) -> List[BoundingBox]:
        ...
        for row in self.token_row_clusters:
            tokens_in_this_row_and_this_sent = [
                token_idx for token_idx in row if token_idx in token_sent_cluster
            ]
    ...
    def _build_sentences(self) -> List[SentenceWithBBoxes]:
        sents = []
        for token_sent_cluster, (sent_start, sent_end) in zip(
            self.token_sent_clusters, self.sent_char_spans
        ):
                ...
                for block_dict in block_dicts:
                    ...
                    block_tokens: List[TokenWithBBox] = [
                        TokenWithBBox(
                            ...
                            bbox=SppBBox.normalize_spp_bbox_json(
                                spp_bbox_json=token_dict["bbox"],
                                page_id=page_id,
                                spp_page_height=page_height,
                                spp_page_width=page_width,
                            ),
                    ...
                    block = Block(
                        tokens=block_tokens, bbox=block_bbox, type=block_dict["type"]
                    )
                    blocks.append(block)
        return blocks
